...
class TreeChart {
  ...
  render() {
    ...
    attrs.root = d3.stratify()
      ...
      .parentId(({
        parentNodeId
      }) => parentNodeId) //, console.log(parentNodeId))
      (attrs.data);

    // console.log(attrs, attrs.data, 'hmjbhmhmhbmhb mfhngvghmhggn grergwgr bvbvcnxbz ');
    // attrs.root = attrs.data.map(d => ({
    //   child: d.id,
    ...
    //     .attr('cy', nodes[i].y)
    //     .attr('r', 20)
    //     .style('fill', nodes[i].backgroundColor);
    // }
  ...
  update({
    ...
    // adding in extra links
    const attrss = this.getChartState();
    const chart = attrss.chart;

    let extraLinks = [];
    const nonHierarchicalLinks = [];
    for (let i = 1; i < nodes.length; i++) {
      
      //   nodes.forEach(node => nonHierarchicalLinks.push(nodes[i].data.otherParent === node.id));
      nodes.forEach(node => {
        if (nodes[i].data.otherParent === node.id) {
        //   nonHierarchicalLinks.push(node);
          nonHierarchicalLinks.push(
            d3.linkHorizontal()({
              source: [nodes[i].x, nodes[i].y],
              target: [node.x, node.y]
            })
          );
        //   nonHierarchicalLinks.push(this.diagonal(nodes[i], node));
        }
      });

      //   nodes.forEach(node => console.log(nodes[i].data.otherParent, node.id));
      //   nonHierarchicalLinks.push(nodes[i].data.otherParent);
      //   extraLinks.push(
      // d3.linkHorizontal()({
      //   source: [nodes[i].x, nodes[i].y],
      //   //   target: nodes[i].parent ? [nodes[i].parent.x, nodes[i].parent.y] : [0, 0]
      //   target: [nodes[i].parent.x, nodes[i].parent.y]
      // })
      //   );
      extraLinks.push(
        console.log('i love dudders', {x: nodes[i].x, y: nodes[i].y}, {x: nodes[i].parent.x, y: nodes[i].parent.y}),
        this.diagonal(nodes[i], nodes[i].parent)
      );
    }
    // console.log('what is links????', extraLinks);
    // console.log('what is nonHierarchicalLinks????', nonHierarchicalLinks);
    ...
    for (let i = 0; i < nonHierarchicalLinks.length; i++) {
      chart
        .append('path')
        .attr('d', nonHierarchicalLinks[i])
        .attr('stroke', 'blue')
        .attr('fill', 'none')
        .attr('transform', `translate(${calc.chartLeftMargin},${calc.chartTopMargin})`);
    }
    ...
    const linkEnter = linkSelection.enter()
      .insert('path', 'g')
      .attr('class', 'link')
    //   .attr('d', i => nonHierarchicalLinks[i])
      .attr('d', d => {
        const o = {
          x: x0,
          y: y0
