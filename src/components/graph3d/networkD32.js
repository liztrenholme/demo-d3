  ...
  useEffect(() => {
    ...
    const node = container.append('g')
      ...
      .join(
        ...
        //   .call(
        //     drag().on('drag', function (evt, d) {
        //       console.log('drag event', evt, d, node);
        //       d.x = evt.x;
        //       d.y = evt.y;
    ...
    function ticked() {
      ...
      svg     
        .call(zoom().on('zoom', (event) => {   // <-- `event` argument
          ...
          svg.select('g').attr('transform', event.transform); // <-- use `event` here
        }))
        .call(
          drag().on('drag', (evt) => {
            console.log('event', evt);
    ...
    if (contextmenuData && Object.keys(contextmenuData).length) {
      console.log(contextmenuData, contextmenuData.x);
